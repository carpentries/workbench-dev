...
Creating tags via the command line is recommended via the `gh` tools.
It is also recommended that your Git configuration is set up to use
[a gpg signature](https://www.garrickadenbuie.com/blog/signed-verified-git-commits-keybase-rstudio/#fnref3)
so that all commits, tags and releases are verified.

:::

Adding a tag and release can be achieved in a single step with the [github
...
The newly released packages will be incorporated into a Docker image via the
[workbench-docker](https://github.com/carpentries/workbench-docker) repository.


## Workbench Release Schedule

As the Workbench is maintained by a small team, the release process is largely
ad-hoc. The capacity of the team to handle issues, fix bugs, and deploy new code
is limited.

However, any release timelines are based on two assumptions:

1. For non-hotfix releases, these will always happen Monday to Thursday
   inclusive. Feature releases will never happen on a Friday.
2. For hotfix releases where critical functionality needs to be restored,
   these may happen at any time.
...
Lesson repositories that use the Workbench workflows in GitHub Actions
will be largely automatically managed.

All lesson builds and updates happen every Tuesday at 00:00 UTC.

Lessons that use R packages for R-based lessons are updated only on the
_first_ Tuesday of each month.

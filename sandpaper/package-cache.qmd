...
file formats. The {renv} package cache allows for R Markdown file formats to be built
...
{renv} is used to form a single "point in time" to refer to when installing
packages for building R Markdown lessons.
...
For even more details on the package cache, see the [Building Lessons With A Package
Cache](https://carpentries.github.io/sandpaper/articles/building-with-renv.html)
article.
...
If the global package cache from {renv} is available, then the lesson profile
(in the lesson `renv/lesson-requirements/profiles/` folder) is activated
before the episode is sent to {knitr}. R will then use the packages
provided in this lesson profile.

## {renv} and GitHub Actions

Whilst the {sandpaper} internals of handling the package cache are the same for
both local and remote builds, the actual package cache are handled differently.

### Docker CI builds

Lessons can employ Continuous Integration on GitHub via GitHub Actions in the
form of workflows.

These workflows provide automated package cache management, and a way to ensure
consistent builds over time where the user decides when to check for and install
updated packages. This has two benefits:

 - Working lesson builds will not suddenly break due to a new base R, system dependency,
   or package versions released with bugs or incompatabilities with other software
 - Pinning package versions to specifically set required functionality, avoiding
   automatic updates where new packages may introduce bugs or remove deprecated functions

[Checks for](https://github.com/carpentries/sandpaper/blob/main/inst/workflows/update-cache.yaml) and
[updatess to](https://github.com/carpentries/sandpaper/blob/main/inst/workflows/docker_apply_cache.yaml)
the package cache on GitHub can be performed with the respective linked workflows.

More information can be found within the
[{sandpaper} workflows readme](https://github.com/carpentries/sandpaper/tree/main/inst/workflows)
documentation.


## A Bit of History

For those interested, this chapter will go into the history of using a package
cache in Carpentries lessons, dig into the design principles, and understand
...
Of course, back in 2016, in order to deploy an R-based lesson it still needed
to be built locally, which sounds simple until you consider the aspect of
...
problems with outputs changing or worse, the
[entire build failing](https://github.com/swcarpentry/r-novice-gapminder/issues/746).
Moreover, lesson maintainers of these R lessons encountered the following
problems:
...
2. Maintainers were unsure of what would happen to the lesson with any given
   ...
   contributor.
3. All the typical struggles with Jekyll and its own system dependencies.
...
### Dockerised solutions

In 2025, following 3 years of production level use of the Workbench in the hands
of lesson developers, maintainers and the Carpentries Core Team, a few issues
were catalogued:

 - Managing lessons on GitHub has quite a high cognitive load, where the complexity
   of the CI, security and build process lent itself to a confusing situation for
   users when things went wrong. Certain things the workflows need to do require
   special setup, or would break due to updated other actions or dependencies.
 - Updates to the base R version or {renv} itself would create quite long-standing
   problems due to package incompatabilities between versions, that can only be
   resolved by upstream package maintainers.
 - Workbench package releases had to be manually recognised and incorporated into a
   lesson rather than being automatically available.

These issues meant that the Core Team had to spend time supporting the
Workbench infrastructure in ways that could be avoided. As such, Rob Davey
undertook the work to make a formal Docker image of R, Rstudio, and the Workbench
which would alleviate these problems by providing a static and consistent build
environment.
...
That means the following with respect to packages in a lesson:
